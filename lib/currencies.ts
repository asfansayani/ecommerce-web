/** ISO 4217 codes with display names (world currencies). */
...
export function isValidCurrency(code: string | null | undefined): code is string {
  ...
  return CURRENCY_SET.has(code.toUpperCase());
}

export function normalizeCurrency(code: string | null | undefined): string {
  if (!code) return DEFAULT_CURRENCY;
  const upper = code.toUpperCase().trim();
  return CURRENCY_SET.has(upper) ? upper : DEFAULT_CURRENCY;
}
...
export function getCurrencyForCountry(
  ...
  return COUNTRY_CURRENCY[upper] ?? DEFAULT_CURRENCY;
}
